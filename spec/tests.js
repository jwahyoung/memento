...
describe('memento core test suite', function () {
    ...
    beforeEach(function() {
        ...
        inject(function(_Memento_) {
            ...
            mementoLimit = new _Memento_(seedObject, 3);
        });
    });
    ...
    it('should have max stack', function () {
        mementoLimit.push({ key: 'value', num: 2 }).then(function () {
            mementoLimit.push({ key: 'value', num: 3 }).then(function () {
                mementoLimit.push({ key: 'value', num: 4 }).then(function () {
                    mementoLimit.push({ key: 'value', num: 5 }).then(function () {
                        mementoLimit.undo().then(function () {
                            mementoLimit.undo().then(function () {
                                mementoLimit.undo().then(function (result) {
                                    expect(result).toEqual({ key: 'value', num: 3 });
                                    mementoLimit.undo().then(function (result2) {
                                        expect(result2).toBeUndefined();
                                    });
                                });
                            });
